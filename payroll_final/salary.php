                                ...
                                <tr class="step-row step-row-<?php echo $grade; ?>" style="display: none;">
                                    ...
                                    <td>
                                        <select class="form-control step-select" 
                                                data-salary-id="<?php echo $step['salary_id']; ?>"
                                                data-salary-grade="<?php echo $step['salary_grade']; ?>"
                                                style="width: 100px;">
                                            <?php for($s = 1; $s <= 8; $s++): ?>
                                                <option value="<?php echo $s; ?>" <?php echo $step['step_no'] == $s ? 'selected' : ''; ?>>
                                                    Step <?php echo $s; ?>
                                                </option>
                                            <?php endfor; ?>
                                        </select>
...
// Handle step change directly in the table
document.addEventListener('DOMContentLoaded', function() {
    ...
    stepSelects.forEach(select => {
        select.addEventListener('change', function() {
            ...
            const salaryGrade = this.dataset.salaryGrade;
            const newStep = this.value;
            
            // Get the new salary rate from the matrix
            let newRate = '';
            if (salaryMatrix[salaryGrade] && salaryMatrix[salaryGrade][newStep - 1]) {
                newRate = salaryMatrix[salaryGrade][newStep - 1].toFixed(2);
            }
            
            if (confirm(`Change to Step ${newStep}?${newRate ? '\n\nNew salary rate will be: ₱' + parseFloat(newRate).toLocaleString('en-PH', {minimumFractionDigits: 2}) : ''}`)) {
                ...
                actionInput.value = 'update_step';
                form.appendChild(actionInput);
                
                const idInput = document.createElement('input');
                idInput.type = 'hidden';
                idInput.name = 'salary_id';
                idInput.value = salaryId;
                form.appendChild(idInput);
                
                const stepInput = document.createElement('input');
                stepInput.type = 'hidden';
                stepInput.name = 'step_no';
