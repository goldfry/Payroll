...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['generate_payroll'])) {
    ...
    // ── PERA limit: ₱1,000 per half-month period, ₱2,000 for full month ──
    ...
    foreach ($selectedDepartments as $deptId) {
        ...
        while ($emp = $employees->fetch_assoc()) {
            ...
            // Calculate deductions (no auto GSIS 9%)
            $gsis       = 0;
    ...
    <div class="period-config-card">
        ...
        <div class="period-grid">
            ...
            <div class="period-field">
                ...
                <input type="number" name="default_pera" id="default_pera"
                       value="1000.00" step="0.01" min="0" max="1000" required>
        ...
        <div class="deductions-section">
            ...
            <p>Note: PhilHealth, Pag-IBIG, and Withholding Tax are calculated automatically based on salary.</p>
...
function updatePeraField() {
    ...
    const peraInput = document.getElementById('default_pera');
    const hint      = document.getElementById('pera-limit-hint');

    peraInput.max   = limit;
    peraInput.value = limit.toFixed(2);
    hint.textContent = 'Max ₱' + limit.toLocaleString('en-PH') + ' for this period';
}
...
function updateCounts() {
    ...
    document.getElementById('selectAllDepts').checked = checkedBoxes.length === allCheckboxes.length;
    document.getElementById('generateBtn').disabled   = checkedBoxes.length === 0;
}
...
document.getElementById('generateForm').addEventListener('submit', function(e) {
    ...
    // Validate PERA against limit
    const limit = getPeraLimit();
    const pera  = parseFloat(document.getElementById('default_pera').value);
    if (pera > limit) {
        e.preventDefault();
        alert('PERA exceeds the ₱' + limit.toLocaleString('en-PH') + ' limit for this period.');
        return false;
    }
    ...
    if (!confirm(`Generate payroll for ${checkedCount} department(s)?\nPeriod: ${month} ${period}, ${year}\n\nExisting records will be skipped.\n\nContinue?`)) {
        e.preventDefault();
        return false;
    }
