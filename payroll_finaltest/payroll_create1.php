...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_batch'])) {
    ...
    foreach ($employeeIds as $empId) {
        ...
        // Per-employee individual loan values
        ...
        // Per-employee values used directly
...
if ($selectedDeptId > 0) {
    ...
    while ($emp = $empResult->fetch_assoc()) {
        ...
        // PRIMARY: get active record from step_increment
        $siQuery = $conn->prepare("
            SELECT si.step_inc, sp.position_title AS si_position
            FROM step_increment si
            LEFT JOIN positions sp ON si.position_id = sp.id
            WHERE si.employee_id = ? AND si.is_active = 'Y'
            LIMIT 1
        ");
...
<?php foreach ($employees as $emp): $eid = $emp['id']; ?>
                    ...
                    <tr class="emp-main-row">
                        ...
                        <td><input type="number" name="basic_salaries[<?php echo $eid;?>]" class="salary-input" value="<?php echo number_format($emp['basic_salary'],2,'.','')?>" step="0.01" min="0"></td>
                        <td><input type="number" name="peras[<?php echo $eid;?>]" class="salary-input pera-input" value="1000.00" step="0.01" min="0" max="1000" style="width:100px" oninput="validatePera(this)"></td>
                    </tr>
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    if (hasSaved) {
        restoreFormState(); // restores saved period + all inputs
    } else {
        updatePeriodOptions('1-31'); // default to full month
        updatePeraLimits();
    }
});
...
function saveFormState() {
    try {
        ...
        // All named inputs (loan fields, basic salary, pera, checkboxes)
...
function restoreFormState() {
    try {
        ...
        // Restore period selectors first
        ['payroll_month','payroll_year'].forEach(id => {
            const el = document.getElementById(id);
            if (el && state[id] !== undefined) el.value = state[id];
        });
        const savedPeriod = state['period_type'] || '1-31';
        updatePeriodOptions(savedPeriod); // rebuild options then set correct value
        ...
        // Recalculate all totals and badges
        document.querySelectorAll('.emp-loan-input').forEach(inp => {
            ...
            if (!subClass) return;
            // class format: emp-{empId}-{prefix}-sub
...
document.getElementById('batchForm').addEventListener('submit', function(e) {
    ...
    clearFormState(); // wipe saved state only on confirmed submit
});
</script>
