...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $statusCheck = $conn->query("SELECT status FROM payroll WHERE id = $id")->fetch_assoc();
    if ($statusCheck && ($statusCheck['status'] === 'Approved' || $statusCheck['status'] === 'Paid')) {
        $_SESSION['error_message'] = 'Cannot edit payroll records with status "' . $statusCheck['status'] . '". Only Draft records can be edited.';
        header('Location: payroll.php');
        exit;
    }
...
if ($payroll['status'] === 'Approved' || $payroll['status'] === 'Paid') {
    $_SESSION['error_message'] = 'Cannot edit payroll records with status "' . $payroll['status'] . '". Only Draft records can be edited.';
    header('Location: payroll.php');
    exit;
}
